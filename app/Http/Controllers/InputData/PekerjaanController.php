    ...
    public function getData(Request $request)
    {
        ...
        if (Auth::user() && Auth::user()->internalRole->uptd) {
            ...
            if(str_contains(Auth::user()->internalRole->role,'Mandor')){
                $pekerjaan = $pekerjaan->where('user_id',Auth::user()->id);
            }else if(Auth::user()->sup_id){
                $pekerjaan = $pekerjaan->where('sup_id',Auth::user()->sup_id);
        ...
        foreach($pekerjaan as $no =>$data){
            ...
            if($detail_adjustment->exists()){
                ...
                if($detail_adjustment->count() > 1){
                    $detail_adjustment=$detail_adjustment->latest('kemandoran_detail_status.created_at')->first();
                ...
                if ($data->input_material) {
                    if ($data->status->status == "Approved") {
                        $approve += 1;
                        // echo $data->status->status.' | '.$it->id_pek.'<br>';
                    } else if ($data->status->status == "Rejected" || $data->status->status == "Edited") {
                        $reject += 1;
                        // echo $data->status->status.' | '.$it->id_pek.'<br>';
                    } else
                        $submit += 1;
            ...
            if ($data->mail == 1) {
                $next_user = DB::table('users')->where('internal_role_id', $data->status->parent)->where('sup_id', $data->status->sup_id)->get();
                // dd($next_user);
                $data->status->next_user = $next_user;

                $name = Str::title($data->status->name);
                $id_pek = $data->id_pek;
                $nama_mandor = Str::title($data->nama_mandor);
                $jenis_pekerjaan = Str::title($data->paket);
                $uptd = Str::upper($data->status->uptd);
                $sup_mail = $data->sup;
                $status_mail = "Submitted";
                $keterangan = "Silahkan menunggu sampai semua menyetujui / Approved";
                $subject = "Status Laporan $data->id_pek-Submitted";
                if (str_contains($data->status->status, 'Edited')) {
                    // dd($item);
                    $status_mail = $data->status->status;
                    $subject = "Status Laporan $data->id_pek-" . $data->status->status;
                }

                $to_email = $data->status->email;
                $to_name = $data->nama_mandor;
                // dd($subject);
                // dd($item);
                ...
                if ($data->status->next_user != "") {
                    foreach ($data->status->next_user as $no => $item1) {
                        // dd($data->email);
                ...
                if ($kemandoran->where('id_pek', $data->id_pek)->exists()) {
                    $mail['mail'] = 2;
        ...
        $sup = $sup->get();
        return view('admin.input.pekerjaan.index', compact('pekerjaan', 'sup', 'mandor',  'sum_report', 'nama_kegiatan_pekerjaan','jenis_laporan_pekerjaan','filter'));
    }
